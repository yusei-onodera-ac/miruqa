...
class RunControllerStopTest {
    ...
    void stopOnDispatchedRunCallsWorkerCancelAndRecordsAudit() {
        ...
        when(workerClient.cancelRun(anyString())).thenReturn(Map.of("runId", "worker-run-1", "status", "cancel_requested"));

        String view = controller.stop(user, "run-1");

        assertEquals("redirect:/runs/run-1", view);
        verify(workerClient, times(1)).cancelRun("worker-run-1");
        verify(auditService, times(1)).record(anyLong(), anyLong(), org.mockito.ArgumentMatchers.eq("run_stop_requested"), anyString());
    }

    @Test
    void stopOnQueuedRunCancelsWithoutContactingWorker() {
        ...
        verify(record, times(1)).setStatus(RunRecord.STATUS_CANCELLED);
        verify(usageService, times(1)).recordIfAbsent(record);
        verify(auditService, times(1)).record(anyLong(), anyLong(), org.mockito.ArgumentMatchers.eq("run_stop_requested"), anyString());
    }
}
