...
- **打ち出し**: 「言わずに去るユーザーの代わりに、先に使ってみるAI」。ユーザーが報告しない小さな不具合を、公開前に先回りして見つける。
...
| 13 | 料金プラン・請求（内部記録） | プランと使用量を見る | 使用量に応じた上限の強制。請求書は内部記録のみ（本物の課金はしない） | ☐（P4） |
...
### 3.11 コスト・使用量・上限（✅ 🟡）
- **台帳 ✅**: すべてのLLM呼び出し（成功も失敗も）を `runs/ledger.jsonl` に記録する（目的・実際に選ばれたモデル・トークン・即時コスト・確定コスト・遅延・再試行・フォールバック段階）。確定額は別行で追記されるため、集計は `python -m agent.cost_report` を使う。
- **予算 ✅**: 呼び出し前に、実行あたり（既定$0.5）・1日あたり（`.env`）の上限を確認し、超えたら**APIを呼ばず**、部分結果で終了する。
- **使用量 🟡**: 実行が終わるたびに、コストを1回だけ記録する（`usage_events`。`run_id` に一意制約で二重計上を防ぐ）。根拠は、ワーカーの台帳の集計。
- **組織の上限 🟡**: 日次（既定$3）・月次（既定$30）の上限。上限に達したら新しい実行を拒否し、80%で警告を表示する。
- **料金プランと、上限の強制・内部の請求書 ☐（P4）**。
