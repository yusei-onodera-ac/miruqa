...
:root { --bg:#f6f7f9; --card:#fff; --ink:#1c2330; --sub:#667085; --line:#e4e7ec; --brand:#3b5bdb;
        --red:#e5484d; --orange:#f08c00; --green:#2f9e44; --blue:#3b82f6; }
...
.sev { display:inline-block; font-size:11px; font-weight:700; color:#fff; padding:2px 9px; border-radius:6px; margin-right:6px; }
.sev-High { background:var(--red); } .sev-Med { background:var(--orange); } .sev-Low { background:#868e96; }
.conf { display:inline-block; font-size:11px; padding:2px 9px; border-radius:6px; margin-right:6px; border:1px solid var(--line); color:var(--sub); }
.conf.confirmed { color:var(--green); border-color:var(--green); }
.lens-tag { display:inline-block; font-size:11px; font-weight:700; padding:2px 8px; border-radius:6px; background:#eef1ff; color:var(--brand); margin-right:6px; }
...
.churn { display:inline-block; font-size:11px; background:#fff4e6; color:#e8590c; border-radius:6px; padding:2px 8px; margin-left:6px; }
...
.chip { font-size:11px; color:var(--sub); background:var(--bg); border-radius:6px; padding:2px 7px; }
.chip.verdict-deny { background:#ffe3e3; color:var(--red); }
.chip.verdict-pending_approval { background:#fff3bf; color:#e8590c; }
...
def _step_card(number, step, runs_root):
  ...
  <span class="chip">{html.escape(obs.get("url") or "")}</span>{chip}</div>
  {img_html}
</div>"""
...
def build_report_html(run: dict, runs_root: Path) -> str:
    ...
    cards = "".join(
        ...
        for l, v in [
            ...
            ("コスト合計(USD)", f'{cost.get("total", 0):.4f}'),
    ...
    findings_html = "".join(_finding_html(f) for f in findings) or '<p class="sub">指摘はありませんでした。</p>'

    cost_rows = "".join(
        f"<tr><td>{html.escape(str(model))}</td><td>${amount:.4f}</td></tr>"
        for model, amount in (cost.get("byModel") or {}).items()
    ) or '<tr><td colspan="2" class="sub">記録なし</td></tr>'

    steps_html = "".join(_step_card(i, s, runs_root) for i, s in enumerate(run.get("steps", []), start=1))
    if not steps_html:
...
<h2>コスト(モデル別・実測値)</h2>
<div class="panel"><table><tr><th>モデル</th><th>コスト(USD)</th></tr>{cost_rows}</table></div>
